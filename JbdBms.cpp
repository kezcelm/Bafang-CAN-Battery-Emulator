...
uint16_t JbdBms::getRatedCapacity(){
  return m_ratedCapacity;
}

uint16_t JbdBms::getResidualCapacity(){
  return m_residualCapacity;
}
...
void JbdBms::parseReqBasicMessage(uint8_t * t_message) {
  ...
  m_residualCapacity = convertTwoIntsToUint16(t_message[8], t_message[9]);
  m_ratedCapacity = convertTwoIntsToUint16(t_message[10], t_message[11]);
